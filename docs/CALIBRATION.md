...
>
> **1 — Frais de gestion GÉNÉRIQUES (branche `appreciation_directe`).** Règle BP réelle
> (vérifiée à l'euro sur Baugé ET Sigoulès) : `frais = 3% × taxes principales + 9% × (TSE et CCI)`.
> - TF  : principales = commune + EPCI + GEMAPI + TEOM ; part 9% = TSE.
> - CFE : principales = CFECommune + CFEEPCI + GEMAPICfe ; part 9% = TSECfe + CCI.
> - ⚠️ Les ex-constantes `FRAIS_TF_EURO=78` / `FRAIS_CFE_EURO=72` étaient les frais de gestion
>   **CALCULÉS de Baugé gelés en dur** (piège « résultat recopié »). Remplacées par
>   `FRAIS_GESTION_PRINCIPALES=0.03` / `FRAIS_GESTION_TSE_CCI=0.09`. Non-régression Baugé : le
>   3%/9% reproduit les 78/72 (TF 2634 / CFE 2169 inchangés, écart < 0.3 €).
>
> **2 — Base cadastrale UNIQUE = CORRECTE (à protéger absolument).** En `appreciation_directe`,
> le BP calcule une ligne « base cadastrale CFE » distincte (abatt 30%, coef 0.15 → ≈ 15 668 €
> pour Sigoulès, ≈ 10 300 € pour Baugé) MAIS **ne l'utilise PAS** : le TOTAL CFE applique la
> base TF (params 50% / 0.26) au rateset CFE. Vérifié sur les formules Excel (TOTAL CFE = SOMME
> des équivalents calculés sur la base TF). → **Le moteur a RAISON avec sa base unique. NE JAMAIS
> réimplémenter de distinction TF/CFE** : ça divergerait du BP réel (donnerait ≈ 4 483 € au lieu
> de la cible Sigoulès 3 264 €).
>
> **3 — Inputs Sigoulès `appreciation_directe` validés.** `methodeTaxes="appreciation_directe"`,
> `baseFonciereKeuro=267.245` (= immo soumises 239 + MOD/aléas retraités 28.245),
> `valeurTerrainKeuro=60` (5000 €/ha × 12 ha). Taux en **décimal** (0.36 = 36%), **NE PAS diviser
> par 100**. Résultat : base cadastrale 11 314 € · **TF 4 586 / CFE 3 267** vs cibles 4 582 / 3 264
> (+0.09%, résiduel = arrondi terrain du BP : 614.56 € écrit là où sa propre formule donne 624 €).
>
...
(⚠️ 78/72 = ANCIENS forfaits Baugé gelés ; désormais CALCULÉS 3% principales / 9% TSE-CCI — voir RÉSOLU FIX 1).
